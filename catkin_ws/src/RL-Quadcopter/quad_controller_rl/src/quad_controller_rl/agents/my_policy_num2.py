"""Dummy agent: Fixed Force in +z direction. Program based on random policy search
                and writing episode stats."""
...
class MyPolicyNum2(BaseAgent):
    """Sample agent that searches for optimal policy dummily."""
    ...
    def __init__(self, task):
        ...
        # Save episode stats
        self.stats_filename = os.path.join(util.get_param('out'),"dummy_stats_{}.csv".format(util.get_timestamp()))  # path to CSV file
        self.episode_num = 1
    ...
    def act(self, state):
        ...
        # Just fix the force in +z direction
        action[0][0] = 0
        action[0][1] = 0
        action[0][2] = 25 # Maximum possible value
        action[0][3] = 0
        action[0][4] = 0
        action[0][5] = 0
