...
class Hover(BaseTask):
    """Simple task where the goal is to maintanin the device at a target height."""
    ...
    def __init__(self):
        ...
        self.target_z = 10.0            # target position to hover at
    ...
    def reset(self):
        ...
        #p = self.target_position + np.random.normal(0.5, 0.1)
        return Pose(
                position=Point(0.0, 0.0, self.target_z + np.random.normal(0.5, 0.1)),  # drop off from a slight random height
    ...
    def update(self, timestamp, pose, angular_velocity, linear_acceleration):
        ...
        if error_position > self.max_error_position:
            print('agent has strayed too much')
            reward -= 50.0  # extra penalty, agent strayed too far
            done = True
        elif timestamp > self.max_duration:  # agent has run out of time
            print('agent has run out of time')
